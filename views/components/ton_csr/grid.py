...
def render_data_grid(selected_ngay, selected_skus):
    ...
    q_data_template = load_query("hang_ton_csr/get_all.sql")
    ...
    if selected_skus:
        # get_all.sql aliases 'ma_barcode' as 'Mã Barcode'
    ...
    total_nhan = int(df_data['Số lượng'].sum()) if 'Số lượng' in df_data.columns else (int(df_data['so_luong'].sum()) if 'so_luong' in df_data.columns else 0)
