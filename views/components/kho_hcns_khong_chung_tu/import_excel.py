"""
views/components/kho_hcns_khong_chung_tu/import_excel.py
Import Excel → bảng kho_hcns_khong_chung_tu (TRUNCATE + INSERT)
"""
...
def render_import_section():
    ...
    with st.expander("⬆️ Import Data từ file Excel", expanded=False):
        ...
        with st.spinner("Đang import dữ liệu..."):
            try:
                SHEET = "DANH SÁCH KHO HCNS KHÔNG CT"
                ...
                for i, row in df_raw.iterrows():
                    ...
                    if any("MÃ BARC" in v or "TÊN SẢN PHẨM" in v or "TỔNG SỐ LƯỢNG" in v for v in vals):
                        header_idx = i
                        break
                ...
                utils_auth.write_log(f"Import {count} SP → kho_hcns_khong_chung_tu (bỏ qua {skip})")
